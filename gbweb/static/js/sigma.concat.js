...
(function() {
    ...
    function Graph() {
        ...
        function subGraph(v, d) {
            var ids = [],
                rem_ids = [],
                next_tier = [];

            /**
             * Keep adding until we reach depth d
             */
            for (i = 0; i < d; i += 1) {
                /** 
                 * Loop over all nodes looking for children of ids
                 */
                next_tier = []
                for (n in self.nodes) {
                    if (self.nodes.hasOwnProperty(n)) {
                        if (i == 0 && self.nodes[n].label == v) { /* Special functionality for d = 0. Just check for label matching. */
                            ids.push(self.nodes[n].id);
                            break;
                        } else { /* Check the edges to see if a source we have has a child of this id */
                            for (e in self.edges) {
                                if (self.edges.hasOwnProperty(e)) {
                                    if (ids.indexOf(self.edges[e].source.id) != -1 && ids.indexOf(self.edges[e].target.id) == -1 && next_tier.indexOf(self.edges[e].target.id) == -1) {
                                        next_tier.push(self.edges[e].target.id);
                                        break; // node_loop;
                                    }
                                }
                            }
                ...
                ids = ids.concat(next_tier);
            }

            for (n in self.nodes) {
                if (self.nodes.hasOwnProperty(n) && ids.indexOf(self.nodes[n].id) == -1) {
                    rem_ids.push(self.nodes[n].id);
                }
            }

            self.dropNode(rem_ids);
        ...
        function dropOrphans() {
            ids = [];
            for (n in self.nodes) {
                if (self.nodes.hasOwnProperty(n) && self.nodes[n].degree == 0) {
                    ids.push(self.nodes[n].id);
                }
            }
            self.dropNode(ids);
        ...
        function dropNodeByLabel(v) {
            for (n in self.nodes) {
                if (self.nodes.hasOwnProperty(n) && self.nodes[n].label == v) {
                    self.dropNode(self.nodes[n].id);
                    break;
                }
            }
            return self;
        ...
        function rescale(w, h, parseNodes, parseEdges) {
            ...
            var scale = self.p.scalingMode == 'outside' ? Math.max(w / Math.max(xMax - xMin, 1),
            h / Math.max(yMax - yMin, 1)) : Math.min(w / Math.max(xMax - xMin, 1),
            ...
            var margin = 40; // (self.p.maxNodeSize || sizeMax) / scale;
            xMax += margin;
            xMin -= margin;
            yMax += margin;
            yMin -= margin;

            scale = self.p.scalingMode == 'outside' ? Math.max(w / Math.max(xMax - xMin, 1),
            h / Math.max(yMax - yMin, 1)) : Math.min(w / Math.max(xMax - xMin, 1),
            h / Math.max(yMax - yMin, 1));
        ...
        function setBorders() {
            ...
            self.nodes.forEach(function(node) {
                ...
                self.borders.minX == undefined ? node['displayX'] - node['displaySize'] : self.borders.minX,
                node['displayX'] - node['displaySize']);

                self.borders.maxX = Math.max(
                self.borders.maxX == undefined ? node['displayX'] + node['displaySize'] : self.borders.maxX,
                node['displayX'] + node['displaySize']);

                self.borders.minY = Math.min(
                self.borders.minY == undefined ? node['displayY'] - node['displaySize'] : self.borders.minY,
                node['displayY'] - node['displaySize']);

                self.borders.maxY = Math.max(
                self.borders.maxY == undefined ? node['displayY'] - node['displaySize'] : self.borders.maxY,
                node['displayY'] - node['displaySize']);
            });
        }
        ...
        this.dropNodeByLabel = dropNodeByLabel;
        this.dropEdge = dropEdge;
        this.dropOrphans = dropOrphans;
    ...
    function MouseCaptor(dom) {
        ...
        function wheelHandler(event) {
            if (self.p.mouseEnabled) {
                ...
                self.mouseX,
                self.mouseY,
                self.ratio * (getDelta(event) > 0 ? self.p.zoomMultiply : 1 / self.p.zoomMultiply));
        ...
        function startInterpolate(x, y, ratio) {
            ...
            progress = self.p.directZooming ? 1 - (isZooming ? self.p.zoomDelta : self.p.dragDelta) : 0;

            if (
            self.ratio != targetRatio || self.stageX != targetStageX || self.stageY != targetStageY) {
                interpolate();
        ...
        function stopInterpolate() {
            ...
            if (isZooming) {
                ...
                self.stageX = targetStageX + (self.stageX - targetStageX) * self.ratio / oldRatio;
                self.stageY = targetStageY + (self.stageY - targetStageY) * self.ratio / oldRatio;
            } else {
    ...
    function Plotter(nodesCtx, edgesCtx, labelsCtx, hoverCtx, graph, w, h) {
        ...
        function task_drawEdge() {
            ...
            while (i++ < self.p.edgesSpeed && self.currentEdgeIndex < c) {
                ...
                s = e['source'];
                t = e['target'];
                if (e['hidden'] || s['hidden'] || t['hidden'] || (!self.isOnScreen(s) && !self.isOnScreen(t))) {
        ...
        function drawEdge(edge) {
            ...
            if (!color) {
                ...
                case 'source':
                    color = edge['source']['color'] || self.p.defaultNodeColor;
                    break;
                case 'target':
                    color = edge['target']['color'] || self.p.defaultNodeColor;
                    break;
                default:
            ...
            case 'curve':
                ...
                ctx.beginPath();
                ctx.moveTo(x1, y1);
                ctx.quadraticCurveTo((x1 + x2) / 2 + (y2 - y1) / 4, (y1 + y2) / 2 + (x1 - x2) / 4,
                x2,
                y2);
                ctx.stroke();
                break;
        ...
        function drawLabel(node) {
            ...
            if (node['displaySize'] >= self.p.labelThreshold || node['forceLabel']) {
                var fontSize = self.p.labelSize == 'fixed' ? self.p.defaultLabelSize : self.p.labelSizeRatio * node['displaySize'];

                ctx.font = self.p.fontStyle + fontSize + 'px ' + self.p.font;

                ctx.fillStyle = self.p.labelColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultLabelColor;
                ctx.fillText(
                node['label'],
        ...
        function drawHoverNode(node) {
            ...
            ctx.font = (self.p.hoverFontStyle || self.p.fontStyle || '') + ' ' + fontSize + 'px ' + (self.p.hoverFont || self.p.font || '');

            ctx.fillStyle = self.p.labelHoverBGColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultHoverLabelBGColor;
            ...
            Math.round(ctx.measureText(node['label']).width + node['displaySize'] * 1.5 + fontSize / 2 + 4),
            ...
            // Node border:
            ctx.beginPath();
            ctx.fillStyle = self.p.nodeBorderColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultNodeBorderColor;
            ...
            // Node:
            ctx.beginPath();
            ctx.fillStyle = self.p.nodeHoverColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultNodeHoverColor;
            ctx.arc(Math.round(node['displayX']),
            Math.round(node['displayY']),
            node['displaySize'],
            0,
            ...
            // Label:
            ctx.fillStyle = self.p.labelHoverColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultLabelHoverColor;
            ctx.fillText(
            node['label'],
            Math.round(node['displayX'] + node['displaySize'] * 1.5),
            Math.round(node['displayY'] + fontSize / 2 - 3));
        ...
        function drawActiveNode(node) {
            ...
            ctx.font = (self.p.activeFontStyle || self.p.fontStyle || '') + ' ' + fontSize + 'px ' + (self.p.activeFont || self.p.font || '');

            ctx.fillStyle = self.p.labelHoverBGColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultActiveLabelBGColor;
            ...
            // Node:
            ctx.beginPath();
            ctx.fillStyle = self.p.nodeActiveColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultNodeActiveColor;
            ctx.arc(Math.round(node['displayX']),
            Math.round(node['displayY']),
            node['displaySize'],
            0,
            ...
            // Label:
            ctx.fillStyle = self.p.labelActiveColor == 'node' ? (node['color'] || self.p.defaultNodeColor) : self.p.defaultLabelActiveColor;
            ctx.fillText(
            node['label'],
            Math.round(node['displayX'] + node['displaySize'] * 1.5),
            Math.round(node['displayY'] + fontSize / 2 - 3));
        ...
        function isOnScreen(node) {
            if (isNaN(node['x']) || isNaN(node['y'])) {
                throw (new Error('A node\'s coordinate is not a ' + 'number (id: ' + node['id'] + ')'));
            }

            return !node['hidden'] && (node['displayX'] + node['displaySize'] > -width / 3) && (node['displayX'] - node['displaySize'] < width * 4 / 3) && (node['displayY'] + node['displaySize'] > -height / 3) && (node['displayY'] - node['displaySize'] < height * 4 / 3);
        };
    ...
    function Sigma(root, id) {
        ...
        this.mousecaptor.bind('drag interpolate', function(e) {
            ...
            self.dispatch(
            e['type'] == 'mousedown' ? 'downgraph' : 'upgraph');
        ...
        function clearSchedule() {
            sigma.chronos.removeTask('node_' + self.id, 2).removeTask('edge_' + self.id, 2).removeTask('label_' + self.id, 2).stopTasks();
            return self;
        };
        ...
        function draw(nodes, edges, labels, safe) {
            ...
            for (var k in self.domElements) {
                if (
                self.domElements[k].nodeName.toLowerCase() == 'canvas' && (params[k] == undefined || params[k] >= 0)) {
            ...
            if (n) {
                if (n > 1) {
                    ...
                    self.plotter.task_drawNode, 'node_' + self.id,
                    false);
            ...
            if (l) {
                if (l > 1) {
                    ...
                    if (previous) {
                        ...
                        self.plotter.task_drawLabel, 'label_' + self.id,
                        previous);
                    } else {
                        sigma.chronos.addTask(
                        self.plotter.task_drawLabel, 'label_' + self.id,
            ...
            if (e) {
                if (e > 1) {
                    ...
                    if (previous) {
                        sigma.chronos.queueTask(
                        self.plotter.task_drawEdge, 'edge_' + self.id,
                        previous);
                    } else {
                        sigma.chronos.addTask(
                        self.plotter.task_drawEdge, 'edge_' + self.id,
    ...
    function SigmaPublic(sigmaInstance) {
        ...
        this.position = function(stageX, stageY, ratio) {
            if (arguments.length == 0) {
                ...
                s.mousecaptor.stageX = stageX != undefined ? stageX : s.mousecaptor.stageX;
                s.mousecaptor.stageY = stageY != undefined ? stageY : s.mousecaptor.stageY;
                s.mousecaptor.ratio = ratio != undefined ? ratio : s.mousecaptor.ratio;
        ...
        this.zoomTo = function(x, y, ratio) {
            ...
            if (ratio == s.mousecaptor.ratio) {
                ...
                s.mousecaptor.interpolate(
                (ratio * x - s.mousecaptor.ratio * s.width / 2) / (ratio - s.mousecaptor.ratio), (ratio * y - s.mousecaptor.ratio * s.height / 2) / (ratio - s.mousecaptor.ratio),
                ratio);
            }
            return self;
        };
        ...
        this.dropNodeByLabel = function(v) {
            s.graph.dropNodeByLabel(v);
            return self;
        };
    ...
    sigma.chronos = new(function() {
        ...
        function queueTask(task, name, parent) {
            ...
            if (!tasks.concat(queuedTasks).some(function(e) {
                ...
                throw new Error('Parent task "' + parent + '" of "' + name + '" is not attached.');
            }
        ...
        function getGeneratorsCount(running) {
            return running ? Object.keys(generators).filter(function(id) {
                return !!generators[id].on;
            }).length : Object.keys(generators).length;
        };
        ...
        function onTaskEnded(e) {
            if (generators[e['content'].taskName] != undefined) {
                if (generators[e['content'].taskName].del || !generators[e['content'].taskName].condition()) {
        ...
        function getFPS() {
            if (isRunning) {
                lastFPS = Math.round(
                framesCount / ((new Date()).getTime() - currentTime) * 10000) / 10;
            }
    ...
    sigma.tools.drawRoundRect = function(ctx, x, y, w, h, ellipse, corners) {
        ...
        var tl = e && (c.indexOf('topleft') >= 0 || c.indexOf('top') >= 0 || c.indexOf('left') >= 0);
        var tr = e && (c.indexOf('topright') >= 0 || c.indexOf('top') >= 0 || c.indexOf('right') >= 0);
        var bl = e && (c.indexOf('bottomleft') >= 0 || c.indexOf('bottom') >= 0 || c.indexOf('left') >= 0);
        var br = e && (c.indexOf('bottomright') >= 0 || c.indexOf('bottom') >= 0 || c.indexOf('right') >= 0);
...
sigma.forceatlas2.ForceAtlas2 = function(graph) {
    ...
    this.atomicGo = function() {
        ...
        case 1:
            ...
            if (self.p.barnesHutOptimize) {
                ...
                while (i < nodes.length && i < self.state.index + cInt) {
                    var n = nodes[i++];
                    if (n.fa2) rootRegion.applyForce(n, Repulsion, barnesHutTheta);
                }
                if (i == nodes.length) {
                    self.state.step = 2;
                    self.state.index = 0;
                ...
                while (i1 < nodes.length && i1 < self.state.index + cInt) {
                    var n1 = nodes[i1++];
                    if (n1.fa2) nodes.forEach(function(n2, i2) {
                        if (i2 < i1 && n2.fa2) {
        ...
        case 2:
            // Gravity
            var Gravity = (self.p.strongGravityMode) ? (self.ForceFactory.getStrongGravity(
            self.p.scalingRatio)) : (self.ForceFactory.buildRepulsion(
            self.p.adjustSizes,
            self.p.scalingRatio));
            ...
            while (i < nodes.length && i < self.state.index + sInt) {
                var n = nodes[i++];
                if (n.fa2) Gravity.apply_g(n, gravity / scalingRatio);
            }
        ...
        case 3:
            ...
            self.p.adjustSizes,
            1 * ((self.p.outboundAttractionDistribution) ? (self.p.outboundAttCompensation) : (1)));
        ...
        case 4:
            ...
            nodes.forEach(function(n) {
                ...
                if (!fixed && n.fa2) {
                    ...
                    totalEffectiveTraction += n.fa2.mass * 0.5 * Math.sqrt(
                    Math.pow(n.fa2.old_dx + n.fa2.dx, 2) + Math.pow(n.fa2.old_dy + n.fa2.dy, 2));
                }
            });
            ...
            var targetSpeed = Math.pow(self.p.jitterTolerance, 2) * self.p.totalEffectiveTraction / self.p.totalSwinging;
            ...
            self.p.speed = self.p.speed + Math.min(
            ...
            if (self.p.adjustSizes) {
                ...
                while (i < nodes.length && i < self.state.index + sInt) {
                    ...
                    if (!fixed && n.fa2) {
                        ...
                        (n.fa2.old_dx - n.fa2.dx) * (n.fa2.old_dx - n.fa2.dx) + (n.fa2.old_dy - n.fa2.dy) * (n.fa2.old_dy - n.fa2.dy));
    ...
    this.ForceFactory = {
        ...
        linRepulsion: function(c) {
            ...
            this.apply_nn = function(n1, n2) {
                if (n1.fa2 && n2.fa2) {
                    ...
                    if (distance > 0) {
                        ...
                        var factor = this.coefficient * n1.fa2.mass * n2.fa2.mass / Math.pow(distance, 2);
            ...
            this.apply_nr = function(n, r) {
                ...
                if (distance > 0) {
                    // NB: factor = force / distance
                    var factor = this.coefficient * n.fa2.mass * r.config('mass') / Math.pow(distance, 2);
        ...
        linRepulsion_antiCollision: function(c) {
            ...
            this.apply_nr = function(n, r) {
                ...
                if (distance > 0) {
                    // NB: factor = force / distance
                    var factor = this.coefficient * n.fa2.mass * r.getMass() / Math.pow(distance, 2);
...
sigma.forceatlas2.Region.prototype.applyForce = function(n, Force, theta) {
    if (this.nodes.length < 2) {
        ...
        var distance = Math.sqrt(
        (n.x - this.p.massCenterX) * (n.x - this.p.massCenterX) + (n.y - this.p.massCenterY) * (n.y - this.p.massCenterY));
...
sigma.publicPrototype.parseGexf = function(gexfPath) {
    ...
    for (i = 0; i < nodesNodes.length; i++) {
        ...
        for (j = 0; j < nodeNodes.length; j++) {
            ...
            sizeNodes = sizeNodes.length ? sizeNodes : nodeNode.getElementsByTagNameNS('*', 'size');
